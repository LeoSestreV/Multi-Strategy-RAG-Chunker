...
def process_file(filepath: str, pipeline: Pipeline, config: PropositionChunkingConfig,
                 output_dir: str) -> str:
    ...
    result = {
        ...
        "strategy": "proposition_based_chunking",
        "timestamp": datetime.now().isoformat(),
        "config": {
            "embedding_model": config.embedding_model,
            "llm_model": config.llm_model,
            "max_propositions_per_chunk": config.max_propositions_per_chunk,
    ...
    out_name = f"{os.path.splitext(filename)[0]}_proposition.json"
...
def main():
    config = PropositionChunkingConfig()
